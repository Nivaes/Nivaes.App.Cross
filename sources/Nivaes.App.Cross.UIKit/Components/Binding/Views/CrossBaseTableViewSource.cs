namespace Nivaes.App.Cross.UIKit
{
    ...
    public abstract class CrossBaseTableViewSource : UITableViewSource
    {
        public event EventHandler SelectedItemChanged;
        private object _selectedItem;
        private readonly WeakReference<UITableView> _tableView;

        protected CrossBaseTableViewSource(UITableView tableView)
        {
            _tableView = new WeakReference<UITableView>(tableView);
        }

        protected CrossBaseTableViewSource(NativeHandle handle)
            : base(handle)
        {
            CrossLogHost.GetLog<CrossBaseTableViewSource>()?.Log(LogLevel.Warning,
...
#pragma warning restore S1168 // Empty arrays and collections should be returned instead of null
        ...
        public virtual void ReloadTableData()
        {
            ...
            {
                CrossLogHost.GetLog<CrossBaseTableViewSource>()?.Log(LogLevel.Warning, exception,
        ...
        {
            if (cell is ICrossBindable bindable)
            {
