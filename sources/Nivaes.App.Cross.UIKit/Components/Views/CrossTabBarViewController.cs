namespace Nivaes.App.Cross.UIKit
{
    ...
    public class CrossTabBarViewController : 
        CrossBaseTabBarViewController, 
        ICrossTabBarViewController
    {
        public CrossTabBarViewController() : base()
        {
        ...
        public CrossTabBarViewController(NSCoder coder) : base(coder)
        {
            // WORKAROUND: UIKit makes a first ViewDidLoad call, because a TabViewController expects it's view (tabs) to be drawn 
            // on construction. Therefore we need to call ViewDidLoad "manually", otherwise ViewModel will be null
            ViewDidLoad();
        ...
        protected CrossTabBarViewController(NSObjectFlag t) : base(t)
        {
        }

        protected internal CrossTabBarViewController(NativeHandle handle) : base(handle)
        {
        }

        public CrossTabBarViewController(string nibName, NSBundle bundle) : base(nibName, bundle)
        {
            // WORKAROUND: UIKit makes a first ViewDidLoad call, because a TabViewController expects it's view (tabs) to be drawn 
            // on construction. Therefore we need to call ViewDidLoad "manually", otherwise ViewModel will be null
            ViewDidLoad();
